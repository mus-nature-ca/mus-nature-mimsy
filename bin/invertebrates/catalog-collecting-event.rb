...
CSV.open(output_dir(__FILE__) + "/catalogs-collecting-events.csv", 'w') do |csv|
  csv << ["Catalog Number", "SiteID", "Site Name", "Catalog Date", "Catalog Collector"]
  catalog_sites.each do |key,val|
    ...
    catalog_ids.in_groups_of(50, false) do |group|
      ...
      catalogs.find_each do |catalog|
        csv << [catalog.catalog_number, site.site_id, site.site_name, catalog.date_collected, catalog.collector]
      end
    end
  end
end