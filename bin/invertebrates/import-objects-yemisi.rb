...
file = "/Users/dshorthouse/Desktop/polychaetes-Final- upload May 07-objects-new.txt"
...
CSV.foreach(file, :headers => true, :col_sep => "\t", :encoding => 'bom|utf-16le:utf-8') do |row|
  ...
  if !obj.nil?
    ...
    #Get the linked site
    site = Site.find_by_site_id(row["SITES.SITE_ID"].strip) rescue nil
    ...
    obj.site = site.site_id rescue nil
    ...
    (1..3).each do |num|
      ...
      if !part.nil?
        ...
        om.om_type = row["OTHER_MEASUREMENTS.OM_TYPE_#{num}"].strip rescue nil
    ...
    #other numbers
    (1..5).each do |num|
      number = row["OTHER_NUMBERS.OTHER_NUMBER#{num}"].strip.squash rescue nil
      if !number.nil?
    ...
    if !obj.credit_line.nil?
      ...
      if !acquisition.nil?
        acq = AcquisitionCatalog.find_by_catalog_number(obj.catalog_number)
        if acq.nil?
