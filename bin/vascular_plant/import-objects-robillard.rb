...
file = "/Users/dshorthouse/Desktop/Pedicularis Data (Erin - August 2015) for David.txt"
log = "/Users/dshorthouse/Desktop/Pedicularis Data (Erin - August 2015) for David-log.csv"
...
CSV.foreach(file, :headers => true, :col_sep => "\t", :encoding => 'bom|utf-16le:utf-8') do |row|
  ...
  #other numbers
  (1..5).each do |num|
    number = row["CATALOGUE.OTHER_NUMBERS.OTHER_NUMBER_#{num}"].strip rescue nil
    if !number.nil?
      other_number = CatalogOtherNumber.new
      other_number.catalog_id = obj.id
      other_number.other_number = number
      other_number.on_type = row["CATALOGUE.OTHER_NUMBERS.ON_TYPE_#{num}"].strip.downcase rescue nil
      other_number.save
    end
  end
