...
file = "/Users/dshorthouse/Desktop/CR object upload BotanyArchiveImages-2018-03-06.txt"
log = "/Users/dshorthouse/Desktop/CR object upload BotanyArchiveImages-2018-03-06-log.csv"
...
CSV.foreach(file, :headers => true, :col_sep => "\t", :encoding => 'bom|utf-16le:utf-8') do |row|
  ...
  obj = Catalog.find_by_catalog_number(row["ID_NUMBER"])

  if !obj.nil?
    new_record = false
    missing << [row["ID_NUMBER"], "Object record already exists"]
    if obj.scientific_name != row["ITEM_NAME"].strip
      # Add to group, "M drive taxonomic update required"
      group_member = GroupMember.new
      group_member.group_id = 4701
  ...
  if new_record
    ...
    #Add to existing Group (id = 4702, "M drive new skeletal records")
    group_member = GroupMember.new
    group_member.group_id = 4702
    group_member.table_key = obj.id
    group_member.save
